...
export class ProxyRestService {

    constructor(
        @Inject(REST_BASE_CONFIG)
        private restBaseConfig: RestBaseConfig,
    ...
    private getRequestByType<T, S>(data: RequestData<S>): Observable<HttpResponse<T> | Object> {
        const currentUrl = this.resolveUrl(data.url, data.options.urlParameters, data.options.encodeUri);
    ...
    private resolveUrl(url: string, options?: Object, encodeUri?: string): string {
        ...
        path = this.restBaseConfig.publicGateway.concat(path);
